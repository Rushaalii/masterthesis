...
data <- read_csv("/Users/tilpommer/Documents/BSE/Term 3/Master project/Synthetic Control/Preschool: PreK Programs/100SyntheticControlWomenNCHLT5.csv.gz")
...
data$laborforce <- ifelse (data$LABFORCE == 2,1,0)
...
pumas2005nyc <- cities2005donorpool$PUMA[cities2005donorpool$Place.Name == "New York city"]
pumas2012nyc <- cities2012donorpool$PUMA[cities2012donorpool$Place.Name == "New York city"]


donorpool <- data[(data$PUMA %in% pumas2005donorpool & data$YEAR < 2012) | (data$PUMA %in% pumas2012donorpool & data$YEAR >= 2012),]

donorpool$nyc <- with(donorpool, ifelse(
  (PUMA %in% pumas2005nyc & YEAR < 2012) | (PUMA %in% pumas2012nyc & YEAR >= 2012),
  1,
  0
))
not_nyc <- donorpool[!(donorpool$PUMA %in% pumas2005nyc & donorpool$YEAR < 2012) | !(donorpool$PUMA %in% pumas2012nyc & donorpool$YEAR >= 2012),]
nyc <- donorpool[(donorpool$PUMA %in% pumas2005nyc & donorpool$YEAR < 2012) | (donorpool$PUMA %in% pumas2012nyc & donorpool$YEAR >= 2012),]
...
###############################
#Descriptive Tables (pre Policy vs after Policy) 
###############################
...
rm(list = ls()) 
data <- read_csv("/Users/tilpommer/Documents/BSE/Term 3/Master project/Synthetic Control/Preschool: PreK Programs/50Synthetic Control_cleaned.csv")
